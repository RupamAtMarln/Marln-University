  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="flex-1 flex flex-col items-center justify-start p-6 overflow-auto">
        {/* Intro/Info Card */}
        <div className="w-full max-w-4xl bg-white rounded-2xl shadow-lg p-8 mt-4 mb-8">
          <div className="flex items-center gap-3 mb-4">
            <BookMarked className="w-8 h-8 text-blue-600" />
            <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">eCollab</h1>
          </div>
          <div className="text-lg text-gray-700 space-y-4">
            <p>
              <span className="font-semibold text-blue-700">What's an eCollab?</span><br />
            ...
            <p>
              <span className="font-semibold text-blue-700">eCollab puts users in control</span> — visibility settings can be set to private or public and adjusted at any time.
            </p>
          </div>
        </div>
        ...
        <div className="w-full max-w-6xl flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1">
            <div className="bg-white rounded-2xl shadow p-6 mb-6">
              ...
              <div className="space-y-6">
                {posts.map(post => (
                  <div key={post.id} className="bg-gray-50 rounded-xl p-4 shadow-sm">
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <div>
                        <div className="font-semibold text-gray-800">{post.user.name} <span className="text-xs text-gray-400">({post.user.role})</span></div>
                        <div className="text-xs text-gray-400">{post.time}</div>
                      </div>
                    </div>
                    <div className="text-gray-700 mb-2 ml-11">{post.content}</div>
                    <div className="ml-11 space-y-2">
                      {post.replies.map(reply => (
                        <div key={reply.id} className="flex items-center gap-2">
                          ...
                          <div className="bg-white rounded-lg px-3 py-1 shadow text-sm">
                            <span className="font-medium text-gray-800">{reply.user.name}</span> <span className="text-xs text-gray-400">{reply.time}</span><br />
                      ...
                      {replyingTo === post.id ? (
                        <div className="flex gap-2 mt-2">
                          ...
                          <button
                            ...
                            className="bg-purple-600 text-white px-3 py-1 rounded-lg flex items-center gap-1 hover:bg-purple-700 text-sm"
                          >
                            <Send size={16} /> Reply
                          </button>
                          <button
                ...
                {posts.length === 0 && (
                  <div className="text-gray-400 text-center py-8">No posts yet. Start the discussion!</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-72 flex-shrink-0 flex flex-col gap-6">
            <div className="bg-white rounded-2xl shadow p-5">
              <h3 className="text-lg font-bold text-gray-800 mb-3 flex items-center gap-2"><Users2 className="w-5 h-5 text-blue-500" /> Quick Links</h3>
            ...
            <div className="bg-white rounded-2xl shadow p-5">
              <h3 className="text-lg font-bold text-gray-800 mb-3 flex items-center gap-2"><Star className="w-5 h-5 text-yellow-500" /> Recent Highlights</h3>
              <ul className="space-y-2 text-sm text-gray-700">
                <li>Emily Brown shared a new project: <span className="font-semibold text-blue-700">Web Dev Portfolio</span></li>
